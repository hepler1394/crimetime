...
export async function GET(request) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/signin?error=${encodeURIComponent(e.message.replace(/^linkMember: /, ""))}`, url.origin));
  }

  if (!member.handle) {
    return NextResponse.redirect(new URL(`/account/handle?next=${encodeURIComponent(next)}`, url.origin));
  }
