...
for (let r = 1; r <= rounds && remaining.length; r++) {
  ...
  if (toRender.length) {
    ...
    if (args.includes("--fal")) run(process.execPath, [join(here, "render-fal.mjs"), ...cloneArgs], "fal render");
    else if (args.includes("--remote")) run(process.execPath, [join(here, "render-remote.mjs"), ...cloneArgs, "--device", "cuda"], "remote render");
